...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4, "Only owner can change dividend contract address");
...
// ------------------------------------------------------------
   ...
   event BodaBnbUnlocked(
    ...
    constructor(address _uniswapRouter02,address _marketWallet,address _dividendToken,address _yieldFarmAddress) ERC20("BODAV2", "BODAV2") {
        ...
        bnbDividendToken = _dividendToken;
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            previousMarketingFee = marketingOrDevelpementFee;
            marketingOrDevelpementFee = 0;
            marketingEnabled = _enabled;
        } else {
            marketingOrDevelpementFee = previousMarketingFee;
            totalFees = marketingOrDevelpementFee.add(bnbDividendRewardsFee);
            marketingEnabled = _enabled;
        }
    ...
    function _transfer(
        ...
        if (
            ...
            bnbDividendRewardsFee = _bnbDividendRewardsFeeBuy;
            marketingOrDevelpementFee = _marketingFeeBuy;
            yieldFarmingFee = _yieldFarmingFeeBuy;
            ...
            if (!swapping && canSwap) {
                ...
                if (marketingEnabled) {
                    ...
                    // uint256 swapTokens = contractTokenBalance.div(totalFees).mul(marketingOrDevelpementFee);
                    // swapTokensForBNB(marketingRewardInContract);
                ...
                if (bnbDividendEnabled) {
                    // uint256 sellTokens = swapTokensAtAmount.add(bnbDividendRewardsFee).mul(bnbDividendRewardsFee);
                    swapAndSendBnbDividends(bnbDividedRewardsInContract);
                    bnbDividedRewardsInContract = 0;
                }
    
                swapping = false;
            }

            bnbDividendRewardsFee = _bnbDividendRewardsFeeSell;
            marketingOrDevelpementFee = _DevelpmentFeeSell;
            yieldFarmingFee = _yieldFarmingFeeSell;
            lpFee = _LpFeeSell;
